...
def parse_task5(base_dir):
    ...
    for run_dir in glob.glob(os.path.join(base_dir, "run_*")):
        ...
        file_path = os.path.join(run_dir, "ability_flee.json")
...
if __name__ == "__main__":
    ...
    parsed = parse_task5(base_dir)
    out_path = os.path.join(os.path.dirname(__file__), "parsed_task5.json")

    with open(out_path, "w") as f:
        json.dump(parsed, f, indent=2)
    print(f"Task 5 parsed data written to {out_path} ({len(parsed)} flees)")
